...
namespace Impostors.Samples
{
    ...
    internal class ExampleSpawner : MonoBehaviour
    {
        ...
        [SerializeField]
        private int _autoSpawnCount = default;
        ...
        [SerializeField]
        Vector3 randoms = new Vector3(5f, 5f, 5f);
        ...
        void Update()
        {
            if (button || IsSpawnButtonDown() && prefabs != null)
            {
                button = false;
                switch (spawnType)
                {
        ...
        private bool IsSpawnButtonDown()
        {
#if ENABLE_INPUT_SYSTEM
            return Keyboard.current != null ? Keyboard.current.cKey.wasPressedThisFrame : false;
        ...
        void OrderSpawn()
        {
            ...
            // x_z
            randomRot = Quaternion.Euler(0, Random.Range(0, 360), 0);
            go = (GameObject)Instantiate(GetRandomPrefab(), transform.position + new Vector3(_orderSpawnCount * offset, 0, _orderSpawnCount * offset),
                randomRot);
            go.transform.parent = this.transform;
            ...
            {
                randomRot = Quaternion.Euler(0, Random.Range(0, 360), 0);
                go = (GameObject)Instantiate(GetRandomPrefab(), transform.position + new Vector3(_orderSpawnCount * offset, 0, i * offset),
                    randomRot);
                go.transform.parent = this.transform;
